...
- (id)initWithWeiboText:(NSString*) weiboText withImage:(UIImage*)image url:(NSString *)url aditionalText:(NSString *)text andDelegate:(id)del
{
    ...
    if (self) {
		...
		[_sendWeiboButton setTitle:@"发送" forState:UIControlStateNormal];
...
-(void)sendBtnTouched:(id)sender
{
    ...
    NSString *tweet;
    if (_additionalText) {
        tweet = [NSString stringWithFormat:@"%@ %@", _weiboContentTextView.text, [_additionalText URLEncodedString]];
    } else
        tweet = _weiboContentTextView.text;
    if (_weiboImageData) {
        [blog upload:tweet imageUlr:_imageUrl delegate:_delegate onSuccess:@selector(postSucceeded) onFail:@selector(postFailed)];
    } else
        [blog update:tweet delegate:_delegate onSuccess:@selector(postSucceeded) onFail:@selector(postFailed)];
}
